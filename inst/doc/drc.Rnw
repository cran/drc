...
plot(modelex2, xlab="Dose", ylab='Rate of Oxygen Evolution', conName="Control", col=TRUE)
...
modelex3.3 <- multdrc(rgr~dose, pct, collapse=data.frame(pct, 1, pct, pct), data=TM, fct=w4())
...
<<sec6-fit4, echo=T>>=
modelex3.4 <- multdrc(rgr~dose, pct, data.frame(pct, pct, pct), data=TM, fct=w3())
@
