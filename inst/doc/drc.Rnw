...
Two models for sigmoidal dose response curves are widely used: The logistic model and the Gompertz model. We will briefly discuss these models. 
...
the relative slope around $e$, and the $e$ parameter is the logarithm of the inflection point. The Gompertz model is not symmetric around any point.
The three-parameter Gompertz model with the lower limit equal to 0 is then

\[
f \big(x, (b,d,e) \big) = d \exp\{ -\exp\{ b(\log (x)- e)\}  \}.
\]
Besides the Gompertz model and the logistic model, Brain-Cousens' model \citep{brain&cousens:1989} is used in situations where \emph{hormesis} is present: 
...
\\
\\
Notice that the $b$ and $e$ parameters are different in (\ref{l4}), (\ref{l5}), (\ref{g4}) and (\ref{bcl4}), respectively.
...
A similar approach can be used for the Gompertz model and Brain-Cousens' model. For another example see section~\ref{sec:7}.
...
\] 
The Gompertz model yields a similar formula. For Brain-Cousens' model there are no closed-form solutions, but numerical methods can be applied; we
...
Function & Model\\
\hline
\verb+g3()+ & Three-parameter Gompertz\\
\verb+g4()+ & Four-parameter Gompertz\\
%\verb+mg4()+ & Modified four-parameter Gompertz\\
...
\Sexpr{as.numeric(format(summary(modelex1)[[1]],digits=2))}.
...
<<sec4-plot1, echo=T, fig=T, height=5, width=5>>=
...
<<sec5-plot1, echo=T, fig=T, height=5, width=5>>=

plotraw(SLOPE~DOSE, CURVE, data=PestSci, ylab='Rate of Oxygen Evolutions', colour=TRUE)
...
<<sec5-plot2, echo=T, fig=T, onefig=T, height=5, width=5>>=

plot(modelex2, xlab="Dose", ylab='Rate of Oxygen Evolution', conName="Control", colour=TRUE)
...
We define a simultaneous model, assuming that each dose response curve follows a four-parameter Gompertz model (\ref{g4})
and with different parameters for different assays. This model is specified as follows

<<sec6-fit1, echo=T>>=

modelex3.1<-multdrc(rgr~dose, pct, data=TM, fct=g4())
...
The asymmetric four-parameter Gompertz model is specified with the argument \verb+fct=g4()+. 
The lack-of-fit test against the two-way ANOVA using \verb+anova+ (not displayed) confirms that the Gompertz model fits the data.
\\
\\
A summary of the fit \verb+modelex3.1+ is

<<sec6-summary1, echo=T>>=
...
\begin{figure}[!htbp]
\begin{center}
<<sec6-plot1, echo=T, fig=T, height=5, width=5>>=
...
The data frame should contain as many columns as there are parameters in the model, eg four columns in case of the four-parameter Gompertz model, 
...
collapse=data.frame(TM[,2], TM[,2], colFct(TM[,2],1:8), TM[,2]), fct=g4())
\end{verbatim}
<<sec6-fit2, echo=F>>=
...
modelex3.3 <- multdrc(rgr~dose, pct, collapse=data.frame(pct, 1, pct, pct), data=TM, fct=g4())
...
can be done using the built-in function \verb+g3()+ which is a three-parameter Gompertz model (lower limit set to 0).

<<sec6-fit4, echo=T>>=
modelex3.4 <- multdrc(rgr~dose, pct, data.frame(pct, pct, pct), data=TM, fct=g3())
@
...
MMfct <- function(x, parm) {parm[,1]*x/(parm[,2]+x)}
...
MMssfct <- function(data) {Vm <- max(data[,2]); K <- Vm*data[1,1]/(data[1,2] - data[1,1]); return(c(Vm, K))}
...
modelex4 <- multdrc(rate~conc, state, data=Puromycin, fct=list(MMfct, MMssfct, MMnames))
...
rm(modelex1, modelex1.boxcox, modelex2, modelex3.1, modelex3.2, modelex3.3, modelex3.4, FA, PestSci, TM)
rm(modelex4, MMfct, MMssfct, MMnames)
detach(package:drc)
