...
FIT3 <- drm(y~x, data = data1, fct = LL.4(ss = "3"), weights = data2[, 2])
summary(FIT3)
plot(FIT3)

FIT4a <- drm(y~x, data=data1, fct=LL.4(fixed=c(6, -5e-9, NA, NA), ss = "3"))
summary(FIT4a)
plot(FIT4a)

FIT4 <- drm(y~x, data=data1, fct=LL.4(fixed=c(6, -5e-9, NA, NA), ss = "3"), weights=data2[, 2])
summary(FIT4)
plot(FIT4)
...
FIT6 <- drm(y~x, data=data1, fct=LL.4(fixed=c(NA,1E-9,5E-8,1E+12), ss = "2"))
summary(FIT6)
