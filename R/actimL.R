"actimL" <- function(
fixed = rep(NA, 14), names = c("b1", "b2", "c1", "c2", "d", "e1", "e2", "f1", "f2", "e3", "f3", "b3", "c3", "g"), ssfct = NULL)
{
    ## Checking arguments
    numParm <- 14
...
#        print(parm)
        ...
        parmMat[, 10] <- parmMat[, 10] / ((dose[, 1]^parmMat[, 14]) * dose[, 2])
        ...
        applyImplicitFct <- function(parmVec)
        {
            if ((!is.finite(parmVec[6])) && (!is.finite(parmVec[7])))
            {
                ...
                implicitFct <- function(oVal)
                {
                    ...
                    (1 / parmVec[10]) * ((oVal^(-1 / parmVec[11]) - 1)^(1 / parmVec[12])) - 1                    
...
#                ((parmVec[4] - parmVec[5]) / parmVec[7]) * (occuprate^(-1 / parmVec[9]) - 1)^(1 / parmVec[2]))
                ...
                ((parmVec[13] + occuprate * (parmVec[5] - parmVec[13])) / parmVec[10]) * (occuprate^(-1 / parmVec[11]) - 1)^(1 / parmVec[12])                
...
#        print(apply(parmMat, 1, applyImplicitFct))                  
    ...
    if (!is.null(ssfct))
    {
        ...
        {
            LLstart <- llogistic()$ssfct(dframe[, c(1, 3)])
            initval <- c(LLstart[c(1, 1, 2, 2, 3, 4, 4)], rep(1, 4), LLstart[c(1, 2)], 1) * c(-1, -1, rep(1, 9), -1, 1, 1)
