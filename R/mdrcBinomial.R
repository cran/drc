"mdrcBinomial" <- function(dose, resp, multCurves, startVec, robustFct, weights, rmNA, zeroTol = 1e-12, anovaYes = FALSE)
{
    ...
    gofTest <- function(resp, weights, fitted, dfres)
    {
        ...
        c( sum( (success - expected)^2 / (expected*(1 - fitted)) ), dfres)  # df.residual(object))    
    }
